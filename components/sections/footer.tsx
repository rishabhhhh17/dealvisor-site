...
export function Footer() {
  return (
    <footer className="relative border-t border-hairline">
      <div className="absolute inset-x-0 -top-px h-px bg-gradient-to-r from-transparent via-dv-blue/40 to-transparent" />
      <div className="dv-container py-16">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-10">
          <div className="lg:col-span-5">
            <Wordmark className="text-fg" />
            <p className="mt-5 max-w-sm text-sm text-fg-muted">
          ...
          <div className="lg:col-span-7 grid grid-cols-3 gap-8">
            {cols.map((c) => (
              <div key={c.title}>
                <div className="text-[11px] font-mono uppercase tracking-[0.14em] text-fg-subtle">{c.title}</div>
                <ul className="mt-4 space-y-2.5 text-sm">
                  {c.links.map((l) => (
                    <li key={l.label}>
                      <a href={l.href} className="text-fg-muted hover:text-fg transition-colors">
                        {l.label}
                      </a>
                    </li>
                  ))}
        ...
        <div className="mt-6 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 text-[11px] font-mono uppercase tracking-[0.14em] text-fg-subtle">
          <span>© 2026 Valence Growth Partners</span>
          <span>Made in Mumbai · London</span>
        </div>
      </div>
    </footer>
  );
