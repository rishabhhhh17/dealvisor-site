      ...
      <div className="dv-container">
        <Reveal>
          ...
          <h2 className="mt-5 max-w-3xl text-display-2 text-balance">
            <span className="text-grad-ink">Banking runs on chaos.</span>{' '}
            <span className="text-grad-cool">We fix that.</span>
          </h2>
          ...
          {items.map(({ icon: Icon, title, body }) => (
            <StaggerItem key={title} variants={revealVariants}>
              <SpotlightCard className="h-full p-6 lg:p-7">
                <span className="inline-flex size-9 items-center justify-center rounded-xl bg-gradient-to-br from-dv-blue/15 via-dv-cyan/10 to-dv-violet/15 border border-hairline">
                ...
                <div className="mt-6 h-px w-full bg-gradient-to-r from-dv-blue/40 via-dv-violet/30 to-transparent" />
              </SpotlightCard>
            </StaggerItem>
          ))}
        </Stagger>
